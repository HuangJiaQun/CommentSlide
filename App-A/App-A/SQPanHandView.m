...
//定义颜色宏
#define ColorRGBA(x, y, z, alp) [UIColor colorWithRed:x/255.0 green:y/255.0 blue:z/255.0 alpha:alp]

@interface SQPanHandView ()<UITableViewDelegate,UITableViewDataSource, UIGestureRecognizerDelegate>
...
@implementation SQPanHandView

+ (instancetype)DromPopViewWithFrame:(CGRect)frame commentBackView:(UIView *)commentBackView {
    SQPanHandView *view = [[SQPanHandView alloc] initWithFrame:frame commentBackView:commentBackView];
    return view;
}
...
- (void)dismiss {
    [UIView animateWithDuration:0.15f
                     ...
                     animations:^{
                         ...
                         frame.origin.y = frame.origin.y + frame.size.height;
                         self.container.frame = frame;
                        //self.backgroundColor = ColorRGBA(0, 0, 0, 0);
...
//        NSLog(@"transP : %@",NSStringFromCGPoint(transP));
        if(self.lastDrapDistance > 5 && self.isDragScrollView == NO) {
            //如果是类似轻扫的那种
            [self dismiss];
        } else {
            //如果是普通拖拽
            if(self.container.frame.origin.y >= [UIScreen mainScreen].bounds.size.height - self.container.frame.size.height*2/5) {
                [self dismiss];
            } else {
                [UIView animateWithDuration:0.15f
                                      delay:0.0f
